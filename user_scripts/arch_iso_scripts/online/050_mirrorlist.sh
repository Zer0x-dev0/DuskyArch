...
# --- ALARM (aarch64) FALLBACK STORE ---
# Arch Linux ARM repos live on their own network ($arch/$repo layout, HTTP only).
# The mainline CDNs above do not host ALARM packages and return 404 for aarch64.
read -r -d '' ALARM_FALLBACK_MIRRORS << 'EOF' || true
## Arch Linux ARM Fallback (Auto-generated by orchestrator)
...
apply_alarm_fallback_mirrorlist() {
    write_text_file_atomic "$TARGET_FILE" 0644 "$ALARM_FALLBACK_MIRRORS"
}
...
sync_arch() {
    ...
    # --- ARCHITECTURE GUARD ---
    # Reflector and the mainline CDN fallbacks target x86_64 mirrors only and
    # 404 on ALARM. aarch64 gets its dedicated mirrorlist instead.
    if [[ "$(uname -m)" == "aarch64" ]]; then
        log_info "ALARM (aarch64) detected - bypassing reflector (x86_64-only mirror network)."
        apply_alarm_fallback_mirrorlist
        log_ok "Arch Linux ARM fallback mirrorlist applied."
